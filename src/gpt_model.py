...
# 2 gram character
chars_3gram = sorted(list(set(re.findall('[a-zA-Z]{3}',text))))
...
def encode_3gram(text):
    encoded_text = []
    i = 0
    while (i < len(text)-2):
        if (text[i]+text[i+1]+text[i+2] in tokens):
            encoded_text.append(stoi[text[i]+text[i+1]+text[i+2]])
            i+=3
        else:
...
data = torch.tensor(encode_3gram(text), dtype = torch.long)
...
print(decode_3gram(m.generate(context, max_new_tokens=2000)[0].tolist()))